...
# Increased from 5.0 to 300.0: the Silero ONNX model's _state and _context are
# fixed-size numpy arrays that do NOT grow, so frequent resets don't help memory.
# Resetting at 5s caused Silero to lose its LSTM context at ~5-second boundaries,
# which broke detection exactly when the bot finished its first (long) greeting.
_MODEL_RESET_STATES_TIME = 300.0
...
class SileroVADAnalyzer(VADAnalyzer):
    ...
    def voice_confidence(self, buffer) -> float:
        ...
        try:
            ...
            # Diagnostic: log confidence when it's above noise floor
            conf_f = float(new_confidence.item())
            if conf_f > 0.1:
                logger.debug(f"Silero confidence={conf_f:.3f} sr={self.sample_rate} samples={len(audio_int16)}")
